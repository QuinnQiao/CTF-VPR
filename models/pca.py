...
class PCA():
    ...
    def train(self, x):
        ...
        L, U = torch.symeig(x2, eigenvectors=True) # x2 = Udiag(L)U', L is ascending
        if (self.pca_n_components < x2.size(0)):
            k_indices = torch.argsort(L, descending=True)[:self.pca_n_components]
            L = torch.index_select(L, 0, k_indices) # copy
            U = torch.index_select(U, 1, k_indices) # copy
    ...
    def infer(self, data):
        ...
        shape = data.size()
        data = data.view(-1, shape[-1], 1, 1)
        output = F.conv2d(data, self.weight, bias=self.bias, stride=1, padding=0).view(-1, self.pca_n_components)

        output = F.normalize(output, p=2, dim=-1) # IMPORTANT!
        return output.view(*shape[:-1], self.pca_n_components)
